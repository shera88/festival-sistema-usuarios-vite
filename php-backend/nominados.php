 ...
 *
 * Sólo para SUPER ADMIN, y gateado en el servidor: esconder la pestaña en el
 * front no protege nada.
 *
 ...
 * así que sin paginar se perdían ~210 y ocho obras quedaban con el promedio
 * incompleto — dejaban de figurar como nominadas sin que nada lo avisara.
 */
function nomTraerTodo($sb, string $tabla, string $qs, int $tanda = 1000, int $desde = 0): array
{
    $todo = [];
...
foreach ($CONSULTAS as $clave => [$tabla, $qs]) {
    ...
    if (count($filas) === 1000) {
        $filas = array_merge($filas, nomTraerTodo($sb, $tabla, $qs, 1000, 1000));
    }
    $datos[$clave] = $filas;
}

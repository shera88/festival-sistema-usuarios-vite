 ...
 * mezclar, así el orden no delata quién ganó qué. No reordenar aquí, ni ordenar
 * la tabla por ninguna columna, ni agregar una columna de nota o de puesto.
 *
 * Es una vista PURA: recibe los datos ya resueltos y no llama a ningún hook de
 * sesión ni de red. Por eso la pueden usar tal cual el portal de participantes
 * y el área de clientes, sin que una copia se quede vieja respecto de la otra.
 */
...
export function NominadosView({ bloques, total, ano, cargando = false, error = false }: Props) {
  ...
  const bajarPdf = async () => {
    ...
    try {
      ...
      await descargarNominadosPdf(bloques, { ano, total });
    } catch (e) {
      ...
      <header className="mb-4 flex flex-wrap items-start justify-between gap-3 px-1">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[12px] text-text-45">
            {total} nominados en {bloques.length} categorías · el orden es aleatorio y no

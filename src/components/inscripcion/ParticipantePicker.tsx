...
/** Forma reducida que devuelve /api/participantes/search — sin PII. */
...
type Props = {
  value?: string;
  onSelect: (participante: Participante | { nombre_y_apellido: string }) => void;
};
...
/** Debounce simple para el input de búsqueda — 250ms es cómodo para tipeo normal. */
...
export function ParticipantePicker({ value, onSelect }: Props) {
  ...
  // Fetch resultados conforme se tipea (mínimo 2 chars)
  useEffect(() => {
    if (!open) return;
    ...
    let aborted = false;
    setLoading(true);
    supabase.rpc("search_participantes", { q }).then(({ data, error }) => {
      if (aborted) return;
      if (error) console.error("[ParticipantePicker] search:", error.message);
  ...
  const handleSelect = async (p: ParticipanteLite) => {
    // Cerramos el panel ya; la UI indica "seleccionado". El fetch de detalle
    // completa los otros campos en segundo plano.
    setOpen(false);
    setSearch("");
    // Informamos al formulario inmediatamente con el nombre — así la validación
    // del campo pasa aunque el detalle aún no haya llegado.
    onSelect({ nombre_y_apellido: p.nombre_y_apellido });
    try {
      const { data, error } = await supabase.rpc("obtener_contacto_por_id", { p_id: p.id_contacto });
      if (error) return;
      const arr = data as Participante[] | null;
      if (arr && arr[0]) onSelect(arr[0]);
    } catch {
      // Silencioso: si falla el detalle, el usuario tendrá que llenar los campos a mano.
    }
  };
  ...
  const handleAddNew = () => {
    ...
    onSelect({ nombre_y_apellido: trimmed });
    setNewName("");
    setAddingNew(false);
    setOpen(false);
  };
        ...
        {value ? (
          ...
          <>
            ...
            <div className="flex-1">
              <div className="text-sm font-semibold text-foreground">Seleccionar su nombre</div>
            ...
            <>
              ...
              <div className="max-h-80 overflow-y-auto">
                {showPrompt ? (
                  <div className="p-6 text-center text-sm text-foreground/70">
                    Empiece a escribir su nombre para ver sugerencias.
                  </div>
                  ...
                  <div className="p-6 text-center">
                    ...
                    >
                      + Registrarme como &ldquo;{search}&rdquo;
                    </button>
                  </div>
                ) : (
                  <ul role="listbox">
                    {results.map((p) => {
                      const selected = value === p.nombre_y_apellido;
                      return (
                        <li key={p.id_contacto}>
                          ...
                          >
                            <Avatar name={p.nombre_y_apellido} size={36} />
              ...
              <div className="border-t border-border bg-card p-3">
                ...
                >
                  ...
                  Registrarme como persona nueva
                </button>
              </div>
            </>
          )}
